...
extern "C" {
    fn DecodeD64(input: *const u8, inputlen: usize, output: *mut u8, outputlen: usize) -> isize;
    fn EncodeD64(input: *const u8, inputlen: usize, output: *mut u8, outputlen: usize) -> isize;
...
pub(crate) fn encode_jaguar(input: &[u8]) -> std::io::Result<Vec<u8>> {
    let mut output = Vec::with_capacity(input.len() * 9 / 8 + 1);
    unsafe {
        let res = EncodeJaguar(
            input.as_ptr(),
            input.len(),
            output.as_mut_ptr(),
            output.capacity(),
...
pub(crate) fn decode_d64<'a, E: ParseError<&'a [u8]>>(
    ...
    unsafe {
        ...
        if res < 0 {
            return Err(crate::nom_fail(input));
        }
        output.set_len(res as usize);
    }
    Ok((&[], output))
...
// broken for now
#[allow(dead_code)]
pub(crate) fn encode_d64(input: &[u8]) -> std::io::Result<Vec<u8>> {
...
#[no_mangle]
pub(crate) extern "C" fn bytevec_new() -> *mut Vec<u8> {
    Box::into_raw(Box::default())
}

#[no_mangle]
pub(crate) unsafe extern "C" fn bytevec_free(buf: *mut Vec<u8>) {
    let _ = Box::from_raw(buf);
}

#[no_mangle]
pub(crate) unsafe extern "C" fn bytevec_len(buf: *const Vec<u8>) -> usize {
    (*buf).len()
}

#[no_mangle]
pub(crate) unsafe extern "C" fn bytevec_get(buf: *const Vec<u8>, i: usize) -> u8 {
    (*buf)[i]
}

#[no_mangle]
pub(crate) unsafe extern "C" fn bytevec_push_back(buf: *mut Vec<u8>, v: u8) {
    (*buf).push(v);
}

#[no_mangle]
pub(crate) unsafe extern "C" fn bytevec_pop_front(buf: *mut Vec<u8>) -> u8 {
    (*buf).remove(0)
}
